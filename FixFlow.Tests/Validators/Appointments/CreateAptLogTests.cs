...
public class CreateAptLogValidatorTests {
	...
	public void Should_Have_Error_When_Price_Is_Negative() {
		var model = new CreateAptLog("clientId", null, DateTime.UtcNow, -1, null, null, DateTime.UtcNow.AddDays(1));
	...
	public void Should_Not_Have_Error_When_Price_Is_Zero_Or_Positive() {
		var model = new CreateAptLog("clientId", null, DateTime.UtcNow, 0, null, null, DateTime.UtcNow.AddDays(1));
		var result = _validator.TestValidate(model);
		result.ShouldNotHaveValidationErrorFor(x => x.Price);

		model = new CreateAptLog("clientId", null, DateTime.UtcNow, 10, null, null, DateTime.UtcNow.AddDays(1));
	...
	public void Should_Have_Error_When_DateTime_Is_Before_2024() {
		var model = new CreateAptLog("clientId", null, new DateTime(2023, 12, 31), 10, null, null, DateTime.UtcNow.AddDays(1));
		var result = _validator.TestValidate(model);
	...
	public void Should_Not_Have_Error_When_DateTime_Is_2024_Or_Later() {
		var model = new CreateAptLog("clientId", null, new DateTime(2024, 1, 1), 10, null, null, DateTime.UtcNow.AddDays(1));
		var result = _validator.TestValidate(model);
